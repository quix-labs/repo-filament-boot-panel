...
use App\Models\User;
use App\Providers\Helpers\Unsecure;
...
class AdminPanelProviderDatabaseWorking extends ServiceProvider
{
    // Instead of using register function to handle my database tenants, I use boot function

    public function register(): void
    {
        // Fix is to force this provider to boot before FilamentServiceProvider and after DatabaseServiceProvider
        $this->app->booting(function (Application $app) {
            Unsecure::reorderProviderBoot(
                app: $app,
                provider: self::class,
                target: FilamentServiceProvider::class,
                position: 'before'
            );
        });
    ...
    public function boot(): void
    {
        ...
        foreach ($tenants as $tenant) {
            Filament::registerPanel(
                fn(): Panel => $this->panel(Panel::make(), $tenant),
            );
        }
    }
